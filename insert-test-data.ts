...
async function insertTestData() {
  ...
  try {
    ...
    if (authError) {
      ...
      console.log('   Continuing with test data insertion...\n');
      
      // Try to get current user or use a dummy ID
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        console.log('❌ No authenticated user. Cannot insert test data.');
        console.log('   Please sign in first or disable email confirmation in Supabase.');
        return;
      }
    } else {
    ...
    if (!userId) {
      console.log('⚠️  No user authenticated. Using mock data insertion...\n');
...
async function insertMockDataDirectly() {
  ...
  if (error) {
    ...
    console.log('\n   Options:');
    console.log('   1. Temporarily disable RLS on tables for testing');
    console.log('   2. Create and sign in with a test user');
    console.log('   3. Modify RLS policies to allow inserts for testing');
  } else {
